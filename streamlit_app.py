...
st.caption("Paste draw history directly, system learns from transitions between draws.")
...
def transition_matrix_to_df(trans: Counter) -> pd.DataFrame:
    ...
    for x in range(10):
        ...
        if row_sum > 0:
            mat[x] /= row_sum
    df = pd.DataFrame(mat, columns=[str(i) for i in range(10)], index=[str(i) for i in range(10)])
    return df
...
def apply_positionless_transitions(seed: Tuple[int, ...], probs: Dict[Tuple[int,int], float], top_k: int = 3) -> List[Tuple[int, ...]]:
    ...
    for v in seed:
        ...
        choices = [y for y, p in dist[:top_k]]
        choices_per_digit.append(choices)
    # Generate all combinations
    candidates = list(it.product(*choices_per_digit))
    return [tuple(cand) for cand in candidates]
...
raw_text = st.text_area(
    "Paste your draw history (latest draw on top). Format: 6543 or 6543-7 if Fireball.",
    height=200
)
...
if raw_text.strip():
    for line in raw_text.strip().splitlines():
        line = line.strip()
        if "-" in line:
            main, fb = line.split("-", 1)
            digs = to_tuple(main)
            if len(digs) == n_digits:
                draws.append(digs)
                fireballs.append(int(fb.strip()) if fb.strip().isdigit() else None)
        else:
            digs = to_tuple(line)
            if len(digs) == n_digits:
                draws.append(digs)
